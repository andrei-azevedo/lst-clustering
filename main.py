...
def processPCAPS(packets_with_tx, packets_without_tx):
    ...
    fig, ax = plt.subplots()
    ax.hist(df[4], color='#908d8e', label=['Regular HTTP traffic'])
...
def main():
    packets_with_tx = getPacketsFromPCAP(os.path.join(os.getcwd(), 'regular_traffic.pcap'))
    # packets_without_tx = getPacketsFromPCAP(os.path.join(os.getcwd(), 'file_without_tx.pcap'))
    processPCAPS(packets_with_tx, None)
